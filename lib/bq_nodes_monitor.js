...
NodesMonitor.prototype.refreshData = function(callback) {
  ...
  try {
    ...
    if(response.statusCode != 200) {
      var error = new Error("Error getting data from [error_type:node_monitor] [cluster_url:"+this.clusterUrl+"] [response_status:"+response.statusCode+"] [response_body:"+response.body+"]");
      log.log("error", error);
      throw error; 
    }
  } catch(e) {
    ...
    return log.log("info", "[error_type:node_monitor] Error getting data from api",e);
  }
  ...
  if(Object.keys(local_nodes).length == 0) {
  //If no nodes found It won't change
    log.log("error", "No nodes found: "+self.clusterUrl);
  } else {
